...
| `pnpm-version` | no | Node only. Empty by default so pnpm/action-setup reads the consumer's `package.json` `packageManager` pin; set a value only for repos without one (passing both errors under action-setup v6) |
...
| pnpm | `11` | Node consumers only. Pinned by each consumer's `package.json` `packageManager` field rather than an action input default, which is empty so action-setup reads that pin. |
